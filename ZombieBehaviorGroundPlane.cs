...
public class ZombieBehaviorGroundPlane : MonoBehaviour {
	...
	void Update () {
		...
		if (!dead) // perform is zombie alive
		{
			...
			{
				// ground plane test
				//Vector3 newDir = Vector3.RotateTowards (transform.forward, transform.position + movement, 0.1f * Time.deltaTime, 0.0f);
				//transform.rotation = Quaternion.LookRotation (newDir);
				//transform.position = Vector3.MoveTowards (transform.position, transform.position + movement, 0.1f * Time.deltaTime);
			}
			else 	// move towards center
			{
				// ground plane test
				//Vector3 newDir = Vector3.RotateTowards (transform.forward, Vector3.zero, 0.1f * Time.deltaTime, 0.0f);
				//transform.rotation = Quaternion.LookRotation (newDir);
				//transform.position = Vector3.MoveTowards (transform.position, Vector3.zero, 0.1f * Time.deltaTime);
			}

			// if out of bounds, set bool to move towards center and add 5 more seconds to timer
			if (!moveTowardsCenter && (transform.position.x > 1 || transform.position.x < -1 || transform.position.z < -1 || transform.position.z > 1) ) 
			{
